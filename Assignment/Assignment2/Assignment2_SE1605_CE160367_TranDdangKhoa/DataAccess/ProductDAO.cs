...
namespace DataAccess
{
    public class ProductDAO : BaseDAL
    {
        ...
        public IEnumerable<ProductObject> GetProductList()
        {
            ...
            string SQLSelect = "Select ProductID, CategoryID, ProductName, Weight, UnitPrice, UnitsInStock from Products";
        ...
        public void AddNew(ProductObject product)
        {
            try
            {
                ...
                if (mo == null)
                {
                    string SQLInsert = "Insert Products values(@CategoryID,@ProductName, @Weight, @UnitPrice, @UnitsInStock)";
        ...
        public void Update(ProductObject product)
        {
            try
            {
                ...
                if (mo != null)
                {
                    string SQLUpdate = "Update Products set CategoryID = @CategoryID,ProductName = @ProductName , Weight = @Weight,UnitPrice= @UnitPrice, UnitsInStock= @UnitsInStock where ProductID = @ProductID";
                    var parameters = new List<SqlParameter>();
        ...
        public void Remove(int ProductID)
        {
            try
            {
                ...
                if (mo != null)
                {
                    string SQLDelete = "Delete Products where ProductID = @ProductID)";
                    var param = dataProvider.CreateParameter("@ProductID", 4, ProductID, DbType.Int32);
