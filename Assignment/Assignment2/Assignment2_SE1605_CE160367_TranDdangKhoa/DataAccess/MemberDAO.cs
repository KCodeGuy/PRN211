...
namespace DataAccess
{
    public class MemberDAO : BaseDAL
    {
        ...
        public MemberObject GetMemberByID(int memberID)
        {
            ...
            IDataReader dataReader = null;
            string SQLSelect = "Select MemberID, Email,CompanyName, City, Country, Password from Members where MemberID = @MemberID;";
            try
            {
        ...
        public void AddNew(MemberObject member)
        {
            try
            {
                ...
                if (mo == null)
                {
                    string SQLInsert = "Insert Members values(@MemberId @Email,@CompanyName, @City, @Country, @Password)";
                    var parameters = new List<SqlParameter>();
                    parameters.Add(dataProvider.CreateParameter("@MemberId", 100, member.MemberID, DbType.Int32));
        ...
        public void Update(MemberObject member)
        {
            try
            {
                ...
                if (mo != null)
                {
                    string SQLUpdate = "Update Members set  Email = @Email ,CompanyName = @CompanyName, City = @City,Country= @Country,Password= @Password where MemberID = @MemberID";
                    var parameters = new List<SqlParameter>();
        ...
        public void Remove(int MemberID)
        {
            try
            {
                ...
                if (mo != null)
                {
                    string SQLDelete = "Delete from Members where MemberID = @MemberID";
                    var param = dataProvider.CreateParameter("@MemberID", 4, MemberID, DbType.Int32);
